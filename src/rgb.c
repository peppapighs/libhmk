...
void rgb_task(void) {
   ...
   * transfers and their interrupts every 20 ms while lighting is disabled or
   * USB is suspended. */
  if (!usb_suspended && rgb_config.enabled != 0u &&
      rgb_config.brightness != 0u &&
      rgb_config.effect != RGB_EFFECT_STATIC &&
      rgb_config.effect != RGB_EFFECT_LIVE &&
  ...
  for (uint16_t i = 0; i < RGB_LED_COUNT; i++) {
    ...
    for (uint8_t channel = 0; channel < RGB_BYTES_PER_PIXEL; channel++) {
      ...
      rgb_output[dst + channel] =
          rgb_config.enabled && !usb_suspended ? (uint8_t)(scaled / 255u)
                                               : (uint8_t)0u;
    }
  }
